...
public:
	...
	inline Room* get_room(ECS::Handle h) { if (h < 0 || h >= (ECS::Handle)rooms.size()) { return nullptr;}  return rooms[h]; }
	inline Room* get_room(int x, int y) { if (x >= width || x < 0 || y < 0 || y >= height) {return nullptr;} return rooms[y * width + x]; }
